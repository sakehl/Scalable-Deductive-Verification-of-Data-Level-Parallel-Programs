...
/*@ context get_num_groups(0) == 1 && get_num_groups(1) == 1 && get_num_groups(2) == 1;
  ...
  context asum_offset >= 0;
  context input != NULL && \pointer_length(input) >= 2*WGS2();
  context asum != NULL && \pointer_length(asum) >= asum_offset+1;
  context \ltid==0 ==> Perm({:asum[asum_offset]:}, write);
  requires Perm({:lm[\ltid]:}, write);
@*/
#ifndef CONST_TYPES
//@ context (\forall* int i; 0 <=i && i<\pointer_length(input); Perm({:input[i]:}, read));
#endif
#if RELAX_WORKGROUP_SIZE == 1
  __kernel
#else
...
#endif
void XasumEpilogue(READ_ONLY1 __global real* restrict input,
                   UNIQUE2 __global real* asum, const int asum_offset) {
