...
object predownloaded extends Module {
  def root = T.input {
    sys.env
      .get("VERCORS_PREDOWNLOADED_DIR")
      .map(path => os.Path(path, os.pwd))
      .getOrElse(os.pwd / "predownloaded")
  }

  def antlrJar = T { root() / "antlr" / "antlr4.jar" }
  def z3Jar = T { root() / "z3" / "com.microsoft.z3-4.8.7.jar" }
  def viperRoot = T { root() / "viper" }
}

object external extends Module {
  private def cachedJar(localJar: os.Path, dest: os.Path, fileName: String, url: String): PathRef = {
    if(os.exists(localJar)) {
      PathRef(localJar)
    } else {
      os.write(dest / fileName, requests.get.stream(url))
  ...
  object z3 extends Module {
    ...
    def classPath = T {
      cachedJar(predownloaded.z3Jar(), T.dest, "z3.jar", url())
    }
  }
  ...
  object antlr extends Module {
    ...
    def classPath = T {
      cachedJar(predownloaded.antlrJar(), T.dest, "antlr.jar", url())
    }
  }
}
...
object viper extends ScalaModule {
  object silverGit extends GitModule {
    ...
    override def predownloadedRepo = T { Some(predownloaded.viperRoot() / "silver") }
    def filteredRepo = T {
      val workspace = repo()
  ...
  object siliconGit extends GitModule {
    ...
    override def predownloadedRepo = T { Some(predownloaded.viperRoot() / "silicon") }
    def filteredRepo = T {
      val workspace = repo()
      os.remove.all(workspace / "src" / "test")
  ...
  object carbonGit extends GitModule {
    ...
    override def predownloadedRepo = T { Some(predownloaded.viperRoot() / "carbon") }
    def filteredRepo = T {
      val workspace = repo()
      os.remove.all(workspace / "src" / "test")
      os.remove(workspace / "src" / "main" / "resources" / "logback.xml")
