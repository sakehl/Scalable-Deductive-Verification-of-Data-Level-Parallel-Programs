...
### Predownloaded dependencies

If you want to build without downloading `antlr`, `z3`, or the Viper repositories during the build, place them under a local `predownloaded/` directory in the repository root, or point `VERCORS_PREDOWNLOADED_DIR` to a different root.
...
When those files or folders exist, Mill uses them instead of downloading from the network.
